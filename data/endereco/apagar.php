...
if(!empty($data->id)){
    ...
    if($endereco->apagar()){
        header("HTTP/1.0 200");
        echo json_encode(array("mensagem"=>"Endereço deletado com sucesso!"));
    }
    else{
        header("HTTP/1.0 400");
    ...
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para apagar o endereço."));
}
?>